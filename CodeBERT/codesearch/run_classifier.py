...
def train(args, train_dataset, model, tokenizer, optimizer, scaler=None):
    """Train the model"""
    ...
    for idx, _ in enumerate(train_iterator):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                if args.fp16:
                    # Unscale the gradients before clipping when using mixed precision
...
def evaluate(
    args, model, tokenizer, checkpoint=None, prefix="", mode="dev", scaler=None
):
...
def main():
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(
            args, train_dataset, model, tokenizer, optimizer, scaler
        )
